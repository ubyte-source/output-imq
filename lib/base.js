(function (window) {
	...
	class Line {
		...
		getTop() {
			...
			let raw = this.getEnergy().getRaw(),
				...
				timer = utilization === this.constructor.saving()
					? raw.report_switcher_saving_power[last][0] - Energy.backtime()
					: raw.report_switcher_bypass_power[0][0];

			let energy = utilization === this.constructor.saving() ? raw.report_switcher_saving : raw.report_switcher_bypass,
				energy_saving = utilization === this.constructor.saving() ? raw.report_switcher_saving_area.toFixed(2).toString() : null,
				energy_bypass_time = utilization === this.constructor.bypass() ? Energy.backtime().toString() : null,
				energy_fields_bitch = utilization === this.constructor.saving() ? Energy.backtime() : raw.report_switcher_bypass_area.toFixed(2);

			let title = utilization === this.constructor.bypass() ? Dictionary.bypass() : Dictionary.saving();
			this.elements.top = this.getEnergy().getIMQ().getTable().getTbody().addRow(
				window.Table.Tr.Td.content(window.moment.unix(timer).utc().format(Energy.format())),
			...
			this.elements.top.getColumn(1).setRowspan(2);
			this.elements.top.getColumn(3).setRowspan(2);
			this.elements.top.getColumn(4).setRowspan(2);
			this.elements.top.getColumn(5).setRowspan(2);
			if (utilization === this.constructor.bypass()) this.elements.top.getColumn(3).setColspan(2);
			for (let item = 5; item < 8; item++) {
		...
		getBottom() {
			...
			let raw = this.getEnergy().getRaw(),
				...
				timer = utilization === this.constructor.saving()
					...
					: raw.report_switcher_bypass_power[0][0] + Energy.backtime();

			let energy = utilization === this.constructor.saving() ? raw.report_switcher_saving + raw.report_switcher_saving_area : raw.report_switcher_bypass + raw.report_switcher_bypass_area;
		...
		getUnderline() {
			...
			this.elements.underline.getColumn(0).setColspan(3);
			this.elements.underline.getColumn(1).setColspan(4);
			this.elements.underline.getColumn(item).setColspan(2);
			this.elements.underline.setClass(color + String.fromCharCode(32) + 'break');
	...
	class IMQ {
		...
		constructor() {
			this.elements = {};
			this.elements.total = new Total(this);
		...
		getHeader() {
			...
			this.elements.header = this.getTable().getThead().addRow(
				window.Table.Tr.Th.content(Dictionary.date()),
				window.Table.Tr.Th.content(Dictionary.mode()),
				window.Table.Tr.Th.content(Dictionary.energy()),
				window.Table.Tr.Th.content(Dictionary.saving()),
				window.Table.Tr.Th.content(Dictionary.bypass()),
				window.Table.Tr.Th.content(Area.saving()),
				window.Table.Tr.Th.content(Area.bypass())
			);
		...
		getUnit() {
			...
			this.elements.unit = this.getTable().getThead().addRow(
				...
				window.Table.Tr.Th.content(null),
				window.Table.Tr.Th.content(String.fromCharCode(91) + Um.power() + String.fromCharCode(93)),
				window.Table.Tr.Th.content(Delta.energy()),
				window.Table.Tr.Th.content(Delta.time()),
				window.Table.Tr.Th.content(Delta.energy()),
				window.Table.Tr.Th.content(Delta.time()),
				window.Table.Tr.Th.content(Delta.energy()),
				window.Table.Tr.Th.content(Delta.time()),
		...
		addSwitch(object) {
			let line = new Energy(this, object);
		...
		generate() {
			...
			for (let item in response) {
				let round = i++ % 2 === 0 ? 2 : 0,
					...
					parameters = [
						window.Table.Tr.Td.content(Report[item]()),
