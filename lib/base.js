(function (window) {
	...
	class Line {
		...
		getUnderline() {
			...
			raw.energy_saving_area_time = raw.report_switcher_bypass_power[0][0] - raw.report_switcher_saving_power[raw.report_switcher_saving_power.length - 1][0];
			...
			this.elements.underline = energy.getIMQ().getTable().getTbody().addRow(
				...
				window.Table.Tr.Td.content(
					utilization === this.constructor.saving()
						? raw.energy_saving_area.toFixed(2).toString()
						: null
				),
				window.Table.Tr.Td.content(
					utilization === this.constructor.saving()
						? raw.energy_saving_area_time
						: raw.energy_bypass_area.toFixed(2).toString()
				),
				window.Table.Tr.Td.content(
